...
class CodeWriter
  def initialize(path_to_asm_file, single_vm)
    @asm_file = File.open(path_to_asm_file, "w")
    write_init unless single_vm
    @function_number = 0
  end
  ...
  def write_call
    ...
    function_init
    write_goto
    label_instruction("RETURN_ADDRESS_#{@parser.arg(1)}") # return address destination
  end

  def write_function
    @function_name = @parser.arg(1)
    puts "function_name = #{@function_name} #{@parser.line_number}\n"
    label_instruction(@parser.arg(1))
  ...
  def write_return
    ...
    a_instruction("RETURN_ADDRESS_#{@function_name}")
    c_instruction("0; JEQ")
    @function_number -= 1
  end
  ...
  def function_init
    ...
    a_instruction("RETURN_ADDRESS_#{@function_name}") # return address
    c_instruction("D=A")
    push_stack
...
private
  def a_instruction(register, set_line_number = false)
    ...
    @asm_file.write("@#{register}#{line_number}\n")
  end
  ...
  def label_instruction(string, set_line_number = false)
    line_number = @parser.line_number if set_line_number
    @asm_file.write("(#{string}#{line_number})\n")
  end
end